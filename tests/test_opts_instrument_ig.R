...
# Basic validation
if (nrow(result) == length(expiry_range)) {
...
for (expiry in expiry_range) {
  epic <- sprintf("OP.D.%s%d.%d%s.IP", instrument, expiry, 5500, "P")
  cat(epic, "\n")
}

# Test with invalid epic names to ensure NA handling
invalid_instrument <- "INVALID"
invalid_result <- opts_instrument_ig(invalid_instrument, expiry_range)

# Verify that all rows in invalid_result have NA values
all_na <- all(is.na(invalid_result$strike_price) & is.na(invalid_result$option_type) & is.na(invalid_result$expiry) & is.na(invalid_result$underlyer))
if (all_na) {
...
final_result <- opts_instrument_ig(instrument, expiry_range)
