...
#'
#' @description A set of helpers for loading and processing options data from the 
#' database.
#'
...
#'
#' @description Extracts options data from the PostgreSQL database and returns it 
#' in a standardized format.
#' This function connects to the database, queries the opts_hist table, and 
#' returns a data frame with
#' standardized column names. The function is vectorized to efficiently handle 
#' multiple expiration 
#' dates using a single SQL query with an IN clause.
#'
#' @param expirDate Optional. A vector of expiration dates to filter by in 
#' 'YYYY-MM-DD' 
#' format as character strings.
#' Example: expirDate <- c(as.character('2024-09-20'), 
#' as.character('2024-10-18')).
#' If NULL, returns all expiration dates. The function is vectorized and uses 
#' an 
#' efficient SQL IN clause.
#'
#' @return A data frame containing the standardized options data with the 
#' following columns:
#' \itemize{
...
#' options_data <- opts_extract_std(expirDate = c(as.character('2024-09-20'), 
#' as.character('2024-10-18')))
#' }
opts_extract_std <- function(expirDate = NULL) {
  ...
  for (date in expirDate) {
    if (!grepl("^\\d{4}-\\d{2}-\\d{2}$", date)) {
      stop("All expirDate elements must be in 'YYYY-MM-DD' format as character 
            strings")
    }
  }
  }
  ...
  tryCatch({
    ...
    names(result) <- c(
      ...
      "callValue", "callVolume", "callOpenInterest", "callBidSize", 
      "callAskSize", "callBidPrice", "callAskPrice", "putValue", "putVolume", 
      "putOpenInterest", "putBidSize", "putAskSize", "putBidPrice", 
      "putAskPrice", "smvVol", "callMidIv", "putMidIv", "delta", "gamma", 
      "theta", "vega", "rho", "phi", "spotPrice"
    )
...
#'
#' @description Creates and tests option instrument names following the IG 
#' naming convention.
#' This function generates epic names for options instruments and retrieves their 
#' details using the OptionsAnalytics package.
#'
#' @param instrument Character string specifying the instrument (e.g., "SPX")
#' @param expiry Numeric vector or range of expiry values (e.g., seq(1, 5))
...
#' \itemize{
#'   \item instrument - The epic code
#'   \item strike_price - The strike price of the option (NA if API call failed)
#'   \item option_type - The type of option (call/put) (NA if API call failed)
#'   \item expiry_ig - The expiry datetime from IG API (NA if API call failed)
#'   \item underlyer - The underlying instrument (NA if API call failed)
#'   \item message - API response message ("success" or error message)
#'   \item expiry_opt - The expiry datetime + 1 day (calculated from expiry_ig)
#'   \item updated - Timestamp of when the data was retrieved (millisecond precision)
#'   \item dte - Days to expiration (calculated from updated and expiry_opt)
#' }
#'
#' @export
#' @examples
...
#' spx_calls <- opts_instrument_ig("SPX", seq(1, 3), strike_price = 5000, 
#' option_type = "C")
#' }
opts_instrument_ig <- function(instrument, expiry_range, strike_price = 5500, option_type = "P") {
  ...
  # Parse expiry_datetime from IG and add 1 day for real options expiry
  expiry_ig_datetime <- as.POSIXct(results$expiry_datetime)
  expiry_opt_datetime <- expiry_ig_datetime + as.difftime(1, units = "days")
  
  results$expiry_opt <- format(expiry_opt_datetime, "%Y-%m-%d %H:%M:%OS3")
  
  # Calculate dte (days to expiration)
  results$dte <- dte_days <- round(as.numeric(difftime(expiry_opt_datetime,
                                                 current_time, units = "days")),
                                   digits = 0)
  
  colnames(results)[4] <- "expiry_ig"
  
  results$updated <- format(current_time, "%Y-%m-%d %H:%M:%OS3")
...
#'
#' @description Retrieves historical options data for specified expiration 
#' dates 
#' with user-friendly limits.
#' This function wraps around \code{opts_extract_std()} to provide historical 
#' analysis capabilities.
#' It sorts results by expiration date and trade date, calculates available 
#' history, and returns
#' the requested number of days of historical data.
#'
#' @param expirDate A vector of expiration dates to filter by in 'YYYY-MM-DD' 
#' format as character strings.
#' Example: expirDate <- c(as.character('2024-09-20'), 
#' as.character('2024-10-18')).
#' @param hist_days An integer specifying the number of days of history to 
#' retrieve.
#' If the available history is less than hist_days, all available rows will be 
#' returned.
#'
#' @return A data frame containing historical options data with the same 
#' columns 
#' as \code{opts_extract_std()},
#' sorted by expirDate and tradeDate in ascending order. The data is limited to 
...
#'   \item Returns the requested number of days (or all available if less than 
#'   requested)
#' }
#'
#' @export
...
#' hist_data <- opts_extract_hist(
#'   expirDate = c(as.character('2024-09-20'), 
#'   as.character('2024-10-18')),
#'   hist_days = 60
#' )
#' }
opts_extract_hist <- function(expirDate, hist_days) {
  ...
  options_data <- options_data[order(options_data$expirDate, 
                                   options_data$tradeDate), ]
  ...
  limited_data <- limited_data[order(limited_data$expirDate, 
                                   limited_data$tradeDate), ]
